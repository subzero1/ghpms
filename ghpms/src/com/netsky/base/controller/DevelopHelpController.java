...
public class DevelopHelpController{
	...
	public void batchAutoCreateJavaXml(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try{
			 ...
			 */
			String eclipse_workspace = new MiniUtils().getEnv("eclipse_workspace");
			...
			String contextName = new RegExp().pickup("/([a-zA-Z_]+[a-zA-Z0-9_]*)/WEB-INF", classpath);
			...
			response.getWriter().print(	"{\"statusCode\":\"300\", \"message\":\""+e.getMessage()+"\"}");
		}
	}
	
	@RequestMapping("/generalFormField.do")
	public void generalFormField(HttpServletRequest request, HttpServletResponse response) throws Exception {
		response.setCharacterEncoding("UTF-8");
		...
		try{
			...
			while(ro.next()){
				...
				try{
					...
					String project_object = convertUtil.toString(ta06.getProject_table());
					String project_table = project_object.substring(project_object.lastIndexOf(".") + 1,project_object.length());
					...
					String detail_objects =  convertUtil.toString(ta06.getDetail_table());
					String aux_objects =  convertUtil.toString(ta06.getAux_table());
					 ...
					 */
					String file_separator = System.getProperty("file.separator");
					formurl = request.getSession().getServletContext().getRealPath("/") + "WEB-INF" + file_separator + "jsp" + file_separator + formurl;
					...
					if(project_table.length() > 0){
						v = new RegExp().pickupAll("name\\s*=\\s*\""+project_table+"\\.([\\S]+)\"",content.toString(),1);
						Map<String,Object> paramMap = new HashMap<String,Object>();
					...
					if(form_table.length() > 0){
						v = new RegExp().pickupAll("name\\s*=\\s*\""+form_table+"\\.([\\S]+)\"",content.toString(),1);
						Map<String,Object> paramMap = new HashMap<String,Object>();
						paramMap.put("module_id", module_id);
					...
					if(detail_objects.length() > 0){
						...
						if(details != null && details.length > 0){
							for(int j = 0;j < details.length;j++){
								...
								String detail_table = detail_object.substring(detail_object.lastIndexOf(".") + 1,detail_object.length());
								v = new RegExp().pickupAll("name\\s*=\\s*\""+detail_table+"\\.([\\S]+)\"",content.toString(),1);
								Map<String,Object> paramMap = new HashMap<String,Object>();
								paramMap.put("module_id", module_id);
					...
					if(aux_objects.length() > 0){
						...
						if(auxs != null && auxs.length > 0){
							for(int j = 0;j < auxs.length;j++){
								...
								String aux_table = aux_object.substring(aux_object.lastIndexOf(".") + 1,aux_object.length());
								v = new RegExp().pickupAll("name\\s*=\\s*\""+aux_table+"\\.([\\S]+)\"",content.toString(),1);
								Map<String,Object> paramMap = new HashMap<String,Object>();
								paramMap.put("module_id", module_id);
	...
	private void setTa07(Vector v,Map paramMap) throws Exception{
		...
		for(int i = 0; i < v.size();i++){
			...
			QueryBuilder queryBuilder = new HibernateQueryBuilder(Ta07_formfield.class);
	...
	public void generalReport(HttpServletRequest request, HttpServletResponse response) throws Exception {
		response.setCharacterEncoding("UTF-8");
		StringBuffer logBox = new StringBuffer("");
		StringBuffer msgBox = new StringBuffer("");
		String statusCode = "200";
		try{
			...
			while(ro.next()){
				...
				try{
					 ...
					 * 备份TA08信息
					 */
					saveService.updateByHSql("update Ta08_reportfield set module_id = module_id*(-1) where module_id = "+module_id);
					 ...
					 * 记录一下生成结果
					 */
					ta06.setExtdesc(convertUtil.toString(ta06.getExtdesc()) + "[统计成功]");
					saveService.save(ta06);
					 ...
					 */
					if(module_id != 0){
						saveService.updateByHSql("delete from  Ta08_reportfield where module_id = "+module_id*(-1));
					}
				}
				catch(Exception ee){
					logBox.append(" error [module_id="+module_id+":"+ee.getMessage()+"]\n\r");
					 ...
					 * 恢复历史数据
					 */
					saveService.updateByHSql("update Ta08_reportfield set module_id = module_id*(-1) where module_id = "+module_id*(-1));
				}
			}
			msgBox.append("成功处理"+counter+"个表的字段");
		}
	...
	private void setTa08(Map paramMap) throws Exception{
		...
		JdbcTemplate jdbcTemplate = jdbcSupport.getJdbcTemplate();
			...
			for(int i = 0;i < list.size();i++){
				...
				else if(t_datatype.indexOf("VARCHAR") != -1 && (t_name.toUpperCase().lastIndexOf("RY") == t_name.length() - 2 || t_comment.indexOf("[人员]") != -1)){
					ta08.setAlign("center");
					Long t_width = t_datalength.longValue() * 3;
					if(t_width < 100){
						t_width = 100L;
				 ...
				 * 类别为2【选项】或 以名称和编号结尾的
				 */
				if(ta08.getSearchtype() == 2 || t_name.toUpperCase().lastIndexOf("MC") == t_name.length() - 2 
						|| t_name.toUpperCase().lastIndexOf("BH") == t_name.length() - 2 ){
					ta08.setShowflag(1L);
				}
	...
	public void generalSaveCfg(HttpServletRequest request, HttpServletResponse response) throws Exception {
		response.setCharacterEncoding("UTF-8");
		StringBuffer logBox = new StringBuffer("");
		StringBuffer msgBox = new StringBuffer("");
		String statusCode = "200";
		try{
			...
			classpath = classpath.replaceAll(classname + ".class", "formSaveModel.model");
			...
			sql.append("and (extdesc not like '%[配置保存成功]%' or extdesc is null )");
			ResultObject ro = queryService.search(sql.toString());
			while(ro.next()){
				Long module_id = null;
				try{
					...
					String form_object = convertUtil.toString(ta06.getForm_table());
					String form_table = form_object.substring(form_object.lastIndexOf(".") + 1,form_object.length());
					String file_name = form_table.substring(form_table.lastIndexOf("_") + 1,form_table.length());
					String detail_objects =  convertUtil.toString(ta06.getDetail_table());
					...
					t_modelContent = t_modelContent.replaceAll("\\$project_object", project_object);
					t_modelContent = t_modelContent.replaceAll("\\$form_object", form_object);
					t_modelContent = t_modelContent.replaceAll("\\$project_table", project_table);
					t_modelContent = t_modelContent.replaceAll("\\$form_table", form_table);
					 ...
					 * 记录一下生成结果
					 */
					ta06.setExtdesc(convertUtil.toString(ta06.getExtdesc()) + "[配置保存成功]");
					saveService.save(ta06);
				}
				catch(Exception ee){
					logBox.append(" error[generalSaveCfg] [module_id="+module_id+":"+ee.getMessage()+"]\n\r");
				}
			}
	...
	public void generalListField(HttpServletRequest request, HttpServletResponse response) throws Exception {
		response.setCharacterEncoding("UTF-8");
		StringBuffer logBox = new StringBuffer("");
		StringBuffer msgBox = new StringBuffer("");
		String statusCode = "200";
		...
		try{
			...
			sql.append("where table_name in('PROJECTINF') and comments is null");
			list = jdbcTemplate.queryForList(sql.toString());
			if(list != null && list.size() > 0){
				Map<String,Object> map = (Map<String,Object>)list.get(0);
				String column_name = convertUtil.toString(map.get("column_name")).toLowerCase();
				throw new Exception ("PROJECTINF 视图中【"+column_name+"】无注释，请补充");
			}
			...
			jdbcTemplate.update("insert into ta07_formfield(id,module_id,name,comments,datatype,datalength,nullable,object_name,width,align,show_flag,search_flag,order_flag,ord) values(ta07_num.nextval,1,'near_status','状态','VARCHAR2',40,'Y','com.netsky.base.flow.vo.DocStruct',50,'left',1,1,1,10002)");
			jdbcTemplate.update("insert into ta07_formfield(id,module_id,name,comments,datatype,datalength,nullable,object_name,width,align,show_flag,search_flag,order_flag,ord) values(ta07_num.nextval,1,'module_name','表单名称','VARCHAR2',100,'N','com.netsky.base.flow.vo.DocStruct',120,'left',1,1,1,1)");
			jdbcTemplate.update("insert into ta07_formfield(id,module_id,name,comments,datatype,datalength,nullable,object_name,width,align,show_flag,search_flag,order_flag,ord) values(ta07_num.nextval,1,'oper_time','处理时间','DATE',14,'Y','com.netsky.base.flow.vo.DocStruct',160,'center',1,1,1,10001)");
			jdbcTemplate.update("insert into ta07_formfield(id,module_id,name,comments,datatype,datalength,nullable,object_name,width,align,show_flag,search_flag,order_flag,ord) values(ta07_num.nextval,1,'near_user','节点','VARCHAR2',70,'Y','com.netsky.base.flow.vo.DocStruct',80,'left',1,1,1,10000)");
