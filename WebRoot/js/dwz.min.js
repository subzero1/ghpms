					...
					$(op.toggleBut, sbar).click(
							function() {
								...
								bar.animate( {
									...
									var cleft = 5 + bar.outerWidth()
											+ $(op.splitBar).outerWidth();
									var cwidth = $(op.container).outerWidth()
											- (cleft - $(op.container).cssv(
													"left"));
									$(op.container).css( {
										left : cleft,
										width : cwidth
...
function initUI(_box) {
	...
	if ($.fn.datepicker) {
		$('input.date', $p).each(function() {
			...
			if ($this.attr("pattern"))
				opts.pattern = $this.attr("pattern");
	...
	if ($.fn.removeSelected)
		$("a[target=removeSelected]", $p).removeSelected();
	if ($.fn.pagerForm)
...
bindings:{
	...
	openTab : function(tabid, url, options) {
		...
		this._init();
		this._scrollCurrent();
		this._getTabs().eq(this._currentIndex).data("url", url).data("tabid",
				tabid);
	}
};
(function($) {
	...
	$.fn.jTable = function(options) {
		return this
				.each(function() {
					...
					$(window).unbind("resizeGrid").bind("resizeGrid",
							_resizeGrid);
				});
	};
	$.jTableTool = {
...
function navTabPageBreak(args) {
	...
	if (args.targetType)
		params[params.length] = args.targetType;
	if (form)
		navTab.reload(form.action, {
			data : $(form).serializeArray(),
...
function dwzPageBreak(args) {
	if (args.targetType == "dialog")
		dialogPageBreak(args);
	else
...
var setting={
						...
						nums$ : "li.j-num>a",
						jumpto$ : "li.jumpto",
						pageNumFrag : '<li class="#liClass#"><a href="javascript:void(0)">#pageNum#</a></li>'
					};
					return this.each(function() {
						var $this = $(this);
		...
		removeSelected : function(options) {
			function _getIds(selectedIds) {
				...
				navTab.getCurrentPanel().find("input:checked").filter(
						"[name=" + selectedIds + "]").each(function(i) {
					var val = $(this).val();
					ids += i == 0 ? val : "," + val;
				});
				return ids;
	...
	$.fn.datepicker = function(opts) {
		...
		function changeTm($input, type) {
			var ivalue = parseInt($input.val()), istart = parseInt($input
					.attr("start")), iend = parseInt($input.attr("end"));
			if (type == 1) {
				if (ivalue < iend) {
					$input.val(ivalue + 1);
				}
				...
				.each(function() {
					...
					function generateCalendar(dp) {
						...
						if (startDay > 0) {
							...
							for ( var t = prevDateWrap.days - startDay; t < prevDateWrap.days; t++) {
								dayStr += '<dd class="other" chMonth="-1" day="'
										+ t + '">' + t + '</dd>';
							}
						}
						for ( var t = 1; t <= dw.days; t++) {
							if (t == dw.day) {
								dayStr += '<dd class="slt" day="' + t + '">'
										+ t + '</dd>';
							} else {
						...
						for ( var t = 1; t <= 42 - startDay - dw.days; t++) {
							dayStr += '<dd class="other" chMonth="1" day="' + t
									+ '">' + t + '</dd>';
						}
						var $days = $(setting.days$).html(dayStr).find("dd");
						$days.click(function() {
							var $day = $(this);
							$this.val(dp.formatDate(dp.changeDay($day
									.attr("day"), $day.attr("chMonth"))));
							if (!dp.hasTime()) {
								closeCalendar();
							} else {
						...
						if (dp.hasTime()) {
							...
							var $minute = $(setting.minute$).val(dw.minute)
									.focus(function() {
										changeTmMenu("mm");
									});
							var $second = $(setting.second$).val(dw.second)
									.focus(function() {
										changeTmMenu("ss");
									});
					...
					$this
							.click(function(event) {
								...
								var yearstart = dw.year
										+ parseInt(dp.get("yearstart"));
								var yearend = dw.year
										+ parseInt(dp.get("yearend"));
	...
	$.extend($.fn, {
		jPanel : function(options) {
			...
			return this.each(function() {
				...
				var footer = $("<div><div></div></div>").appendTo($panel)
						.addClass(op.footer);
												...
												.click(
														function() {
															...
															var property = $(
																	":hidden",
																	box);
															if (property.val() != $this
																	.attr("value")) {
																var change = eval(selector
																		.attr("change"));
																if ($
																		.isFunction(change)) {
																	var param = box
						...
						combox : function() {
							return this
									.each(function() {
										...
										var select = "<div class=\"combox\"><div id=\""
												+ cid
												+ "\" class=\"select\""
												+ (ref ? " rel=\"" + ref : "")
												+ "\" name=\""
										...
										select += "<a href=\"#\" name=\""
												...
												+ value
												+ "\" change=\""
												+ ($this.attr("change") ? $this
														.attr("change") : "")
												+ "\">" + label
												+ "</a></div></div>";
										var options = "<ul class=\"comboxop\" id=\"op"
												+ cid + "\">";
										$("option", $this)
												.each(
														function() {
