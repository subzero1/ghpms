											...
											<div style="display:none;">
												<table width="300" border="0" cellspacing="0" cellpadding="0" class="data-table2" style="border-collapse:collapse;">
													...
													<tr>
														...
														<td width="210"><input name="${obj.id }" class="td-input" <c:if test="${obj.comments=='管理部门'&&user.login_id!='admin' }">value="${xqbms }"</c:if>  style="width:220px;" <c:if test="${(user.login_id=='admin'&&obj.comments=='管理部门')||obj.comments!='管理部门' }"> title="双击选择${obj.comments }" ondblclick="javascript:$.pdialog.open('${obj.selecturl }&name='+this.name+'&module_id='+${obj.module_id }, 'sel_key', '选择${obj.comments }', {mask:true,width:500,height:400});</c:if>"/></td>			
													</tr>
													</c:forEach>
												</table>
											</div>
